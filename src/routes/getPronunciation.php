...
$app->post('/api/IBMWatsonTTS/getPronunciation', function ($request, $response, $args) {
    ...
    $query_str = $settings['api_url'] . '/v1/pronunciation';
    
    $query['text'] = $post_data['args']['text'];
    if(!empty($post_data['args']['voice'])) {    
        $query['voice'] = $post_data['args']['voice'];
    }
    if(!empty($post_data['args']['format'])) {    
        $query['format'] = $post_data['args']['format'];
    }
